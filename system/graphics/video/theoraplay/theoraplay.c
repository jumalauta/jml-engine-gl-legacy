...
typedef struct TheoraDecoder
{
    ...
    volatile long int streamdecodecursor;
    double decodetime;
...
static void WorkerThread(TheoraDecoder *ctx)
{
    ...
    while (!ctx->halt && bos)
    {
        ...
        while ( (!ctx->halt) && (ogg_sync_pageout(&sync, &page) > 0) )
        {
            ...
            else if (decode_audio && !vpackets && (vorbis_synthesis_headerin(&vinfo, &vcomment, &packet) >= 0))
            {
    ...
    while ((!ctx->halt) && ((tpackets && (tpackets < 3)) || (vpackets && (vpackets < 3))))
    {
        ...
        while (decode_audio && !ctx->halt && vpackets && (vpackets < 3))
        {
    ...
    if (!ctx->halt && tpackets)
    {
        ...
        if (tinfo.pixel_fmt != TH_PF_420) { assert(0 && "Only TH_PF_420 pixel format supported"); goto cleanup; } // !!! FIXME
    ...
    ctx->streamdecodecursor = ctx->io->tell(ctx->io);
    ctx->decodetime = 0.0;
    //float old_decodetime = ctx->decodetime;
    ...
    while (!ctx->halt && !eos)
    {
        ...
        /*if (old_decodetime > ctx->decodetime+0.01)
        {
            printf("cleanup now! %.3f > %.3f",old_decodetime, ctx->decodetime);
            goto cleanup;
        }

        old_decodetime = ctx->decodetime;*/

        if (ctx->videolisttail && ctx->videolisttail->playms/1000.0 > ctx->decodetime)
        {
            sleepms(10);
            continue;
            //goto cleanup;
        ...
        if (ctx->decodetime < videoframes-1)
        {
            ctx->io->seek(ctx->io, ctx->streamdecodecursor, 0);
            videoframes = 0;
            continue;
        }*/
        ...
        if (!ctx->halt && tpackets)
        {
            ...
            else
            {
                ...
                {
                    //if (videoframes < ctx->decodetime-1)
                    {
                        unsigned int playms = (fps == 0) ? 0 : (unsigned int) ((((double) videoframes) / fps) * 1000.0);
                        if (abs(ctx->decodetime-playms/1000.0) < 0.1 || !ctx->videolisttail)
                        {
                            ...
                            if (th_decode_ycbcr_out(tdec, ycbcr) == 0)
                            {
                                ...
                                assert(item);
                                if (item == NULL) goto cleanup;
                                item->filecursor = ctx->io->tell(ctx->io);
                                item->playms = playms;
                                item->fps = fps;
        ...
        if (!ctx->halt && need_pages)
        {
            ...
            {
                while (!ctx->halt && (ogg_sync_pageout(&sync, &page) > 0)) {
                    // FIXME ref: https://github.com/godotengine/godot-modules/blob/master/theoraplayer/src/Theora/TheoraVideoClip_Theora.cpp
                    // FIXME: https://www.theora.org/doc/libtheora-1.0/group__basefuncs.html#g707e1e281de788af0df39ef00f3fb432
                    // "Converts a granule position to an absolute time in seconds"
                    // double th_granule_time     (      void *      _encdec, ogg_int64_t      _granpos )
                    // int ogg_sync_pageseek(&sync, &page) // This is useful when seeking within a bitstream. ogg_sync_pageseek will synchronize to the next page in the bitstream and return information about how many bytes we advanced or skipped in order to do so. 
                    ogg_int64_t granule = ogg_page_granulepos(&page);
                    /*double time =*/ th_granule_time(tdec, granule);
...
static long IoFopenRead(THEORAPLAY_Io *io, void *buf, long buflen)
{
    ...
    const long br = fread(buf, 1, buflen, f);
    if ((br == -1) && ferror(f))
        return -1;
    return (long) br;
} // IoFopenRead

static long IoFopenSeek(THEORAPLAY_Io *io, long offset, int relative)
{
    FILE *f = (FILE *) io->userdata;
    ...
    const size_t br = fseek(f, offset, origin);
    if ((br == 0) && ferror(f))
        return -1;
    return (long) br;
} // IoFopenSeek
...
static long int IoFopenCurrentCursor(THEORAPLAY_Io *io)
{
    FILE *f = (FILE *) io->userdata;
    const long int cursor = ftell(f);
    return cursor;
} // IoFopenCurrentCursor
...
void THEORAPLAY_pauseDecode(THEORAPLAY_Decoder *decoder)
{
...
void THEORAPLAY_stopDecode(THEORAPLAY_Decoder *decoder)
{
    ...
    while (videolist)
    {
        ...
        if (videolist->pixels != NULL)
        {
            free(videolist->pixels);
            videolist->pixels = NULL;
        }
        free(videolist);
        videolist = next;
    } // while
...
void THEORAPLAY_setDecodeTime(THEORAPLAY_Decoder *decoder, const double time)
{
    TheoraDecoder *ctx = (TheoraDecoder *) decoder;
    ctx->decodetime = time;
}
